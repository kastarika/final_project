...
def edit(request):
    if(request.method == 'POST'):
        ...
        last_img = aks.objects.latest('id')
        cur = Image.open(last_img.img.path)
        if(edit_type == 'crop'):
            cur.crop((int(request.POST['crop_left']), int(request.POST['crop_up']), int(request.POST['crop_right']), int(request.POST['crop_down']))).save(last_img.img.path)
        if(edit_type == 'resize'):
            cur.resize((int(request.POST['resize_width']), int(request.POST['resize_height']))).save(last_img.img.path)
